...
def visualize_results_by_subset_size(metrics, string_to_add="", save_path="plots/"):
    ...
    for metric_name in metric_names:
        ...
        for approach, values in metrics.items():
            print(f"Approach: {approach}, X: {values['subset_sizes']}, Y: {values[metric_name]}")
            if metric_name in values:
                plt.plot(values["subset_sizes"], values[metric_name], label=approach)
        ...
        plt.legend()
        plt.grid()
        plt.savefig(os.path.join(save_path, f"{metric_name}_comparison{string_to_add}_by_subset_size.png"))
        plt.close()
